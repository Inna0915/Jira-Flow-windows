...
### v1.10.0 (2025-02-21)
- **自动更新功能**:
  - 应用启动后自动检查更新（延迟30秒，避免影响启动性能）
  - 设置页面新增自动更新控制面板
  - 支持手动检查更新、下载更新、重启安装
  - 实时显示下载进度条和状态提示
  - 开发模式下自动禁用（显示"开发模式"标签）
  - 基于 electron-updater 实现，从 GitHub Releases 获取更新

### v1.9.0 (2025-02-16)
- **侧弹窗任务编辑**:
  - 故事点和截止日期编辑从卡片迁移到任务详情侧弹窗
  - Jira 看板可编辑故事点和截止日期
  - 个人看板仅可编辑截止日期（个人任务无故事点概念）
  - 新增保存/取消按钮，点击保存后才触发 Jira API 同步
  - 实时检测修改状态，未保存时显示操作按钮
- **AG Grid 历史数据管理**:
  - History 页面升级为 AG Grid 数据网格（Quartz 主题）
  - 支持 inline 单元格编辑（双击编辑，Enter 保存）
  - 支持列筛选和排序
  - 支持批量删除（勾选多行）
  - 支持新增行（创建个人任务）
  - 现代化的表格 UI 设计（卡片式容器、状态徽章）
- **UI 优化**:
  - 输入框样式优化，提升可读性
  - 选中行高亮效果
